...
exports.listAll = function(req, res) {
  ...
  Transaction_Type.getAll(req,offset,pageSize,function(err, transaction_type) {
  ...
  if (err){
      ...
      Transaction_Type.totalCount(req,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
            totalCount=total[0].TotalCount;
            var result={records:transaction_type,pageNo:pageNo,pageSize:pageSize,TotalRecord:totalCount};
...
exports.search = function(req, res) {
	helper.checkPermission(req,"v",function (isPermited) {
        ...
        var offset = (pageNo - 1) * pageSize;
  Transaction_Type.search(req,req.params.searchKey,offset,pageSize,function(err, transaction_type) {

  if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
       var totalCount=0;
		  Transaction_Type.totalSearchCount(req,req.params.searchKey,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
...
helper.checkPermission(req,"v",function (isPermited) {
        if(isPermited){
  Transaction_Type.getById(req,req.params.transaction_type_id, function(err, transaction_type) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record found",transaction_type[0]));
	}
  });
  } else{
            res.status(403).send(helper.createResponse(helper.Error,0,helper.authError,""));
...
helper.checkPermission(req,"u",function (isPermited) {
        if(isPermited){
  Transaction_Type.updateById(req,req.params.transaction_type_id, new Transaction_Type(req,req.body), function(err, transaction_type) {
     if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
...
helper.checkPermission(req,"d",function (isPermited) {
        if(isPermited){
  Transaction_Type.remove(req,req.params.transaction_type_id, function(err, transaction_type) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
