...
exports.listAll = function(req, res) {
  ...
  Price_Type.getAll(req,offset,pageSize,function(err, price_type) {
  ...
  if (err){
      ...
      Price_Type.totalCount(req,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
            totalCount=total[0].TotalCount;
            var result={records:price_type,pageNo:pageNo,pageSize:pageSize,TotalRecord:totalCount};
...
exports.search = function(req, res) {
	helper.checkPermission(req,"v",function (isPermited) {
        ...
        var offset = (pageNo - 1) * pageSize;
  Price_Type.search(req,req.params.searchKey,offset,pageSize,function(err, price_type) {

  if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
       var totalCount=0;
		  Price_Type.totalSearchCount(req,req.params.searchKey,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
...
helper.checkPermission(req,"v",function (isPermited) {
        if(isPermited){
  Price_Type.getById(req,req.params.price_type_id, function(err, price_type) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record found",price_type[0]));
	}
  });
  } else{
            res.status(403).send(helper.createResponse(helper.Error,0,helper.authError,""));
...
helper.checkPermission(req,"u",function (isPermited) {
        if(isPermited){
  Price_Type.updateById(req,req.params.price_type_id, new Price_Type(req,req.body), function(err, price_type) {
     if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
...
helper.checkPermission(req,"d",function (isPermited) {
        if(isPermited){
  Price_Type.remove(req,req.params.price_type_id, function(err, price_type) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
