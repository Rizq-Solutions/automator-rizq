...
exports.listAll = function(req, res) {
  ...
  Item.getAll(req,offset,pageSize,function(err, item) {
  ...
  if (err){
      ...
      Item.totalCount(req,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
            totalCount=total[0].TotalCount;
            var result={records:item,pageNo:pageNo,pageSize:pageSize,TotalRecord:totalCount};
...
exports.search = function(req, res) {
	helper.checkPermission(req,"v",function (isPermited) {
        ...
        var offset = (pageNo - 1) * pageSize;
  Item.search(req,req.params.searchKey,offset,pageSize,function(err, item) {

  if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
       var totalCount=0;
		  Item.totalSearchCount(req,req.params.searchKey,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
...
helper.checkPermission(req,"a",function (isPermited) {
        if(isPermited){
  var reqObj = new Item(req,req.body);
   if(!reqObj.item_title || !reqObj.item_category_id || !reqObj.creation_date || !reqObj.last_updated_by || !reqObj.last_update_date){
  ...
  Item.create(req,reqObj, function(err, item) {
    
	  if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record Created",item));
	}
  });
}
} else{
...
helper.checkPermission(req,"v",function (isPermited) {
        if(isPermited){
  Item.getById(req,req.params.item_id, function(err, item) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record found",item[0]));
	}
  });
  } else{
            res.status(403).send(helper.createResponse(helper.Error,0,helper.authError,""));
...
helper.checkPermission(req,"u",function (isPermited) {
        if(isPermited){
  Item.updateById(req,req.params.item_id, new Item(req,req.body), function(err, item) {
     if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
...
helper.checkPermission(req,"d",function (isPermited) {
        if(isPermited){
  Item.remove(req,req.params.item_id, function(err, item) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
