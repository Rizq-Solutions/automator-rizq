...
var City = require('../models/cityModel.js');
...
exports.listAll = function(req, res) {
  ...
  City.getAll(req,offset,pageSize,function(err, city) {
  ...
  if (err){
      ...
      City.totalCount(req,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
            totalCount=total[0].TotalCount;
            var result={records:city,pageNo:pageNo,pageSize:pageSize,TotalRecord:totalCount};
...
exports.search = function(req, res) {
	helper.checkPermission(req,"v",function (isPermited) {
        ...
        var offset = (pageNo - 1) * pageSize;
  City.search(req,req.params.searchKey,offset,pageSize,function(err, city) {

  if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
       var totalCount=0;
		  City.totalSearchCount(req,req.params.searchKey,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
...
helper.checkPermission(req,"a",function (isPermited) {
        if(isPermited){
  var reqObj = new City(req,req.body);
   if(!reqObj.city || !reqObj.created_by || !reqObj.last_updated_by){
  ...
  City.create(req,reqObj, function(err, city) {
    
	  if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record Created",city));
	}
  });
}
} else{
...
helper.checkPermission(req,"v",function (isPermited) {
        if(isPermited){
  City.getById(req,req.params.city_id, function(err, city) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record found",city[0]));
	}
  });
  } else{
            res.status(403).send(helper.createResponse(helper.Error,0,helper.authError,""));
...
helper.checkPermission(req,"u",function (isPermited) {
        if(isPermited){
  City.updateById(req,req.params.city_id, new City(req,req.body), function(err, city) {
     if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
...
helper.checkPermission(req,"d",function (isPermited) {
        if(isPermited){
  City.remove(req,req.params.city_id, function(err, city) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
