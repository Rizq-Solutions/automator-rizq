...
exports.listAll = function(req, res) {
  ...
  Transaction.getAll(req,offset,pageSize,function(err, transaction) {
  ...
  if (err){
      ...
      Transaction.totalCount(req,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
            totalCount=total[0].TotalCount;
            var result={records:transaction,pageNo:pageNo,pageSize:pageSize,TotalRecord:totalCount};
...
exports.search = function(req, res) {
	helper.checkPermission(req,"v",function (isPermited) {
        ...
        var offset = (pageNo - 1) * pageSize;
  Transaction.search(req,req.params.searchKey,offset,pageSize,function(err, transaction) {

  if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
       var totalCount=0;
		  Transaction.totalSearchCount(req,req.params.searchKey,function(err, total) {
			if (err){ 
             res.status(200).send(helper.createResponse(helper.Error,0,err,""));
            }else{ 
            if(total && total[0] && total[0].TotalCount && total[0].TotalCount>0){
...
helper.checkPermission(req,"v",function (isPermited) {
        if(isPermited){
  Transaction.getById(req,req.params.transaction_id, function(err, transaction) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record found",transaction[0]));
	}
  });
  } else{
            res.status(403).send(helper.createResponse(helper.Error,0,helper.authError,""));
...
helper.checkPermission(req,"u",function (isPermited) {
        if(isPermited){
  Transaction.updateById(req,req.params.transaction_id, new Transaction(req,req.body), function(err, transaction) {
     if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
...
helper.checkPermission(req,"d",function (isPermited) {
        if(isPermited){
  Transaction.remove(req,req.params.transaction_id, function(err, transaction) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
